...
R has many useful ways of viewing data. Say we want to see how many unique values there are for the categorical variables.

```{r}
count(UFO, city) %>% datatable()
count(UFO, state) %>% datatable()
count(UFO, country)
count(UFO, shape)%>% datatable()
```
We notice that not all of the column D `country` data is complete the 3rd row says `tx` but doesn't have a country value. 


If we look up "Lackland afb" we see that it is indeed a real air force base in Texas in the United States. 


Let's try to save these rows where we have state data but we dont have country data.
...
Let's take a look at more of these situations:

```{r}
filter(UFO, state %in% lower_states & is.na(country)) %>% datatable()
```
...
Now let's say we wanted only sightings from this October 2010 in the US. This could be kinda tricky in Excel or Google Sheets.
